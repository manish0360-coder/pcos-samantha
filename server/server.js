...
const { collectEvidence } = require("./intelligence/memory_federation");
...
io.on("connection", (socket) => {
    ...
    socket.on("ask_question", async (question) => {
        ...
        try {
            // 1. Intelligence Layer: Parse intent
            const symbolicIntent = parseIntentToFilters(question);
            
            // 2. Federation Layer: Collect all evidence
            const evidence = await collectEvidence({ symbolicIntent });
